...
We want to know how many hours each student slept. We use the columns: 'ToSleep' and 'WakeUp'. Create a new column in the data folder. Examine the data again.
...
Suppose you want to compare two samples from two populations (*independent* of each other, and they have different variances). Suppose each sample is **not** normally distributed.

*Use a Wilcoxon-rank Sum test.*

Suppose you want to compare two samples from the same population (so they have the same variance). Suppose each sample is **not** normally distributed.
